...
def solicitud_revision_view(request):
    """Vista para el formulario de solicitud de revisión"""
    if request.method == 'POST':
        ...
        if not all([nombre, apellido, cedula, descripcion]):
            messages.error(request, 'Todos los campos son obligatorios.')
            return render(request, 'mi_app/solicitud_revision.html')
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute("""
                SELECT nombre, apellido, plan_seleccionado, direccion, barrio, telefono, correo
                FROM clientes
                WHERE cedula = %s
                """, [cedula])
                result = cursor.fetchone()
                ...
                if result:
                    ...
                    # Si no está en clientes, buscar en solicitudes aprobadas
                    cursor.execute("""
                    SELECT nombre, apellido, plan_seleccionado, direccion, barrio, telefono, correo
                    FROM solicitudes
            ...
            messages.error(request, 'Error al procesar la solicitud. Por favor intente de nuevo más tarde.')
            return render(request, 'mi_app/solicitud_revision.html')
        
        # Enviar correo al técnico
        try:
            # Preparar asunto y mensaje
            asunto = f'Nueva Solicitud de Revisión - Cliente: {cedula}'
            
            # Contenido del mensaje
            if cliente_encontrado:
                mensaje = f"""Se ha recibido una nueva solicitud de revisión técnica:
                
Datos del cliente:
- Nombre: {cliente_data['nombre']} {cliente_data['apellido']}
- Cédula: {cedula}
- Dirección: {cliente_data['direccion']}, Barrio {cliente_data['barrio']}
- Teléfono: {cliente_data['telefono']}
- Correo: {cliente_data['correo']}
- Plan contratado: {cliente_data['plan']}

Descripción del problema:
{descripcion}

Por favor coordinar la visita técnica lo antes posible.

Saludos,
Sistema InterServicesSM"""
            else:
                # Si no se encontró el cliente en la base de datos
                mensaje = f"""Se ha recibido una nueva solicitud de revisión técnica:
...
NOTA: Este cliente no se encuentra registrado en la base de datos o no tiene una instalación activa.
Se recomienda verificar la información antes de programar la visita.
...
Sistema InterServicesSM"""
            ...
            # Enviar el correo
            from_email = settings.DEFAULT_FROM_EMAIL
            recipient_list = ['fredisjuve@gmail.com']  # Correo del encargado de reparaciones
            
            send_mail(asunto, mensaje, from_email, recipient_list)
            
            # Redirigir con mensaje de éxito
            messages.success(request, 'Su solicitud de revisión ha sido enviada correctamente. En breve nos pondremos en contacto con usted.')
            return redirect('index')
            
        except Exception as e:
            print(f"Error al enviar correo: {str(e)}")
            messages.error(request, 'Error al enviar la solicitud. Por favor intente de nuevo más tarde.')
            return render(request, 'mi_app/solicitud_revision.html')
...
def api_aprobadas(request, cedula):
    """
    Endpoint de API para obtener datos de un posible cliente por su número de cédula
    Retorna nombre, apellido, fecha de posible instalación y plan a contratar
    URL: /api/aprobadas/<cedula>/
    Método: GET
    """
    try:
        # Verificar si es una solicitud GET
        ...
        # Buscar primero en la tabla de solicitudes (instalaciones aprobadas)
        with connection.cursor() as cursor:
            cursor.execute("""
            ...
            # Si no está en ninguna tabla
            return JsonResponse({"error": "Posible cliente no encontrado"}, status=404)
